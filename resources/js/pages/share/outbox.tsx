...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Shares',
        href: route('inbox-shares.shares'),
    },
];
...
const OutBox = ({ filterSort, items, receivers }: { filterSort: any }) => {
    ...
    useEffect(() => {
        ...
        router.visit(route('inbox-shares.shares'), {
            method: 'get',
            data: {
                ...
                <div className="container mx-auto bg-cover bg-center px-4 py-4">
                    <div className="flex flex-wrap items-center justify-between">
                        <h1 className="mb-4 text-2xl font-bold drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]">
                            Shares ({`${items.meta.total} ${items.meta.total > 1 ? 'Items' : 'Item'}`})
                        </h1>
                    </div>
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                    <div className="space-y-1">
                                                        ...
                                                        <div className="justfy-between flex items-center gap-3">
                                                            <Select
                                                                value={filterProp.receiver ? `${filterProp.receiver}` : ''}
                                                                onValueChange={(value) => {
                                                                    setFilterProp({ ...filterProp, receiver: value });
                                                                }}
                                                            >
                                                                <SelectTrigger>
                                                                    ...
                                                                    {receivers.length > 0 &&
                                                                        receivers.map((r) => (
                                                                            <SelectItem value={`${r.email}`} key={`receiver-${r.email}`}>
                                                                                <div className="flex items-center gap-2">{r.email}</div>
                                                                            </SelectItem>
                                                                        ))}
                                                                </SelectContent>
                                                            </Select>
