...
import { dateTimeFormat, IsKeyInObject } from '@/lib/utils';
import { DiaryListingItemProp, SharedOrReceivedDataItem } from '@/types/types';
...
export function DiaryCard({ card, type, from, email }: { card: SharedOrReceivedDataItem; type: 'share' | 'receive'; from: string; email?: string }) {
    const maxVisibleCategories = 2;
    const [showAllCategories, setShowAllCategories] = useState<boolean>(false);
    const diaryItem = card.item as DiaryListingItemProp;
    return (
        <Card className="relative h-64 w-full overflow-hidden">
            <CardContent className="flex h-full flex-col px-4">
                ...
                >
                    ...
                    <p className="mt-2 line-clamp-2 text-sm text-gray-600">{diaryItem.content}</p>
                </Link>

                {/* Categories row with overflow handling */}
                <div className="mt-4 flex items-center gap-1 overflow-hidden">
                    {diaryItem.categories.slice(0, maxVisibleCategories).map((category, index) => (
                        <Badge key={index} variant="outline" className="text-xs">
                            {category.name}
                        </Badge>
                    ))}

                    {diaryItem.categories.length > maxVisibleCategories && (
                        <>
                            ...
                            <Dialog open={showAllCategories} onOpenChange={setShowAllCategories}>
                                <DialogContent className="sm:max-w-md">
                                    ...
                                    <div className="mt-4 flex flex-wrap gap-2">
                                        {diaryItem.categories.map((category, index) => (
                ...
                <div className="mt-auto flex flex-wrap items-center justify-between gap-3 text-sm text-gray-500">
                    <div className="flex items-center gap-1">
                        {diaryItem.emotion?.emoji ?? ''}
                        <span className="ml-2">{diaryItem.emotion?.name ?? ''}</span>
                    </div>

                    <div className="flex items-center gap-3">
                        {diaryItem.files.total > 0 && (
                            <TooltipProvider>
                                <Tooltip>
                                    <TooltipTrigger className="flex cursor-pointer items-center">
                                        <Paperclip className="mr-1 h-3.5 w-3.5" />
                                        <span>{diaryItem.files.total}</span>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                        <ul className="max-w-md list-inside list-none space-y-1 text-gray-200 dark:text-gray-700">
                                            {Object.keys(diaryItem.files.countsByTypes).map((key) => (
                                                <li key={key}>
                                                    {diaryItem.files.countsByTypes[key]} {key}
                                                </li>
                                            ))}
                                        </ul>
                                    </TooltipContent>
